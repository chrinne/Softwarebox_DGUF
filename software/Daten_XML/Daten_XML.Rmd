...
Das Wurzel-Element des Beispieles ist ```\<text>...\</text>``` mit den weiteren Elementen "satz" und "wort" die neben den *start tags* ebenfalls zugehörige *end tags* besitzen. Die Element "wort" beinhaltet konkrete Werte, ebenso "satz" mit dem Punkt. Das Element "text" besitzt das Attribut "autor" und das Element "wort" das  Attribut "typ". Dabei ist das Attribut "typ" für "wort" gut gewählt, da es eine eindeutig Zusatzinformation ist während wir für einen Text auch mehrere Autoren erwarten können und deshalb mit den Elementen "autoren" und "autor" statt einem Attribut arbeiten sollten.  
...
Wenn Sie wohlgeformte komplexere XML-Dateien in einem Editor öffnen, z.B. KML-Dateien, sehen Sie als erstes eine Deklaration und auch Verweise auf anzuwendende Regelwerke. 
...
Diese Beispieldaten haben keine zureichende Strukturierung für die Daten einer realen Ausgrabung.

## XPath 

Mit XPath können die Knoten des Baumes adressiert werden, Rückgabe sind die Elemente und Werte ab der entsprechenden Ordnung (*preorder*). Tragen Sie bitte nacheinander und einzeln folgende XPath-Anweisungen in den Editor ein und führen diese aus:
...
- Die Datierungen mit einem Attribut "methode".
...
- Die Liste der Knoten "f_typ" zum vorangehenden Filter.
...
Auch unscharfe Suchen sind möglich.
```
//feature[dating[matches(text(), "^Final", "i")]]
```
matches, contains
