...
Von Ihrem Studium wissen Sie, unsere Darlegungen müssen nachvollziehbar sein. Auch deshalb zitieren wir die Aussagen von Dritten, führen eine Liste der zitierten Literatur und ergänzen Kataloge mit den verwendeten Daten. In einer digitalen  Arbeitsumgebung sollten wir diese Grundlage wissenschaftlichen Arbeitens angemessen umsetzen. Die Arbeit mit Scriptsprachen wie R für die Analysen und das Bereitstellen dieses Codes und der zugehörigen Daten in einem öffentlichen Repositorium erfüllt diesen Anspruch vollumpfänglich und ist *up to date*.  

## GIT-Plattform und GIT-Account

Neben den bekannten Onlineanbietern (s. Grundlegende Informationen) bieten etliche Universitäten selber GIT an. Informieren Sie sich zuerst bei Ihrem Rechenzentrum. Richten Sie sich in dem von Ihnen gewählten System ein Konto ein. In diesem Konto wird der öffentliche Teile Ihres SSH-Schlüssel abgelegt, nur so können Sie aus Anwendungen wie GIT Extensions und RStudio Daten synchronisieren. 

Beachten Sie bitte: kostenlose Konten sind oft öffentlich.
...
Dann benötigen Sie [GIT](https://git-scm.com/download/win). GIT nutzt keinen eigenen Editor sondern fragt nach einem Verweis auf ein existierendes Programm. Ich nutze hier den Editor  [Notepad++](https://notepad-plus-plus.org/) den Sie ggf. vorab installieren sollten. 
...
Ergänzend zu den Informationen der eingangs genannten Anleitung folgende Hinweise:
 ...
 - Haben Sie GIT bereits konfiguriert, auch über GIT Extensions (s.o.) brauchen Sie das *Git Setup* nicht erneut ausführen. Um zu testen, ob Ihr Name und E-Mail korrekt hinterlegt sind öffnen Sie die Git Kommandozeile (Git Bash) und schreiben Sie folgende Befehle: ```git config --get user.name``` und dann ```git config --get user.email```. Rückabe sollten Ihr Name und die E-Mail-Adresse sein.
